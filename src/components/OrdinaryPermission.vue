<template>
    <div class="Permission">
        <div id="rulePermission" style="border-right:4px #C6E2FF solid;padding:2px">
            <div>
        ...
        <div id="permissionContent" style="padding:2px" v-if="rowClickShow">
            ...
            <div id="userPermission" style="border-top:darkgrey solid 4px;padding:2px;">
                <div style="">
...
export default {
    ...
    data(){
        return{
            ...
            columnsTopInformation:[
                {width: 40, titleAlign: 'center',columnAlign:'center',type: 'selection'},
                {field: 'quanzongChina', title:'权限范围', width: 200, titleAlign: 'center',columnAlign:'center',isResize:true},
                ...
                {field: 'startAtChina', title:'开始时间', width: 30, titleAlign: 'center',columnAlign:'center',isResize:true,},
                {field: 'endAtChina', title:'结束时间', width: 30, titleAlign: 'center',columnAlign:'center',isResize:true,},
            ],
            columnsTopUser:[
                {width: 40, titleAlign: 'center',columnAlign:'center',type: 'selection'},
                {field: 'username', title:'用户名', width: 100, titleAlign: 'center',columnAlign:'center',isResize:true},
    ...
    created(){
        this.$axios.post("/api/api/templetTree/node/getNodesByRoot",{

            }).then((res)=>{
                this.treeNodesData=res.data.nodes
            }).catch((err)=>{
    ...
    mounted(){
        ...
        this.$axios.post(
            '/api/api/permission/ordinaryRule/select',
            {
    ...
    methods:{
        ...
        rowClick(rowIndex,rowData,column){
            ...
            this.$axios.post(
                ...
                (res)=>{
                    // console.log(res)
                    // console.log("1111111",this.getNodeParent("7bc49675dcc64b7e9f60ba0a9bcaa94a",0))   
                    console.log("保存的树节点",this.treeNodesData);
                    console.log("权限内容：",res.data);
                    this.tableDataInformation=res.data.ruleAuthorities;
                    for(var i=0;i<this.tableDataInformation.length;i++){
                        this.tableDataInformation[i].quanzongChina=this.getNodeParent(this.tableDataInformation[i].templetTreeId);
                         //this.getNodeParent(this.tableDataInformation[i].templetTreeId);
                        //break;
                        //
        ...
        addRuleData(){
            if((this.ruleDefine.ruleName=='')||(this.ruleDefine.ruleDescription=='')){
                ...
                this.$axios.post(
                    '/api/api/permission/ordinaryRule/add',
                    {
        ...
        infAddClick(){
            ...
            if(this.count==1){
                this.$axios.post("/api/api/templetTree/node/getNodesByRoot",{
                    ...
                    console.log(this.treeNodesData);
                    this.treeNodesData.map((item)=>{
                        item.children = [];
                        if(item.parentNodeId == null || item.parentNodeId == undefined){
                            this.$refs.tree.append(item,null);
                            console.log("添加全宗节点");
                        }else{
                            console.log("添加子节点")
                            let parent = this.$refs.tree.getNode(item.parentNodeId);
        ...
        getNodeParent(id){
            //console.log(this.treeNodesData)
            // console.log("递归")
            let temArr = [].concat(this.treeNodesData);
            let targetNode = temArr.filter(item=>{
                return item.id ==id;
            })[0];
            if(targetNode.parentNodeId == undefined || targetNode.parentNodeId == ""){
                return targetNode.nodeName;
            }else{
                
                return this.getNodeParent(targetNode.parentNodeId)+"/"+targetNode.nodeName;
            }
        },
        addInfData(){
            let treeIds=[];
            ...
            if((this.informationDefine.startAt=='')&&(this.informationDefine.endAt!='')){
                ...
                if(this.informationDefine.authorityType==2){
                    ...
                    if(treeIds==[]){
                        this.operation('error','权限范围不能为空');
                    }else{
                        if(this.$refs.tree.getCheckedNodes().length>1){
                            this.operation('error','您只能选择一个节点');
                        }else if(this.$refs.tree.getCheckedNodes().length==1){
                            
                            treeIds.push(this.$refs.tree.getCheckedNodes()[0].id)
                        
                        // for(var i=0;i<this.$refs.tree.getCheckedNodes().length;i++){
                        //     treeIds.push(this.$refs.tree.getCheckedNodes()[i].id)

                        // }
                            console.log(treeIds);
                            this.$axios.post(
                                '/api/api/permission/ordinaryAuthority/add',
                                {
                                    ruleId:this.informationDefine.ruleId,
                                    treeId:treeIds[0],
                                ...
                                (res)=>{
                                    ...
                                    if(res.data.success==1){
                                        ...
                                        quanzongChina=this.getNodeParent(treeIds[j])
                                        this.tableDataInformation.push(
                                            {
                                                id:res.data.id,
                                                ruleId:this.informationDefine.ruleId,
                                                templetTreeId:treeIds[j],
                                        ...
                                        this.operation('error',res.data.msg+'，添加失败')
                                    }
        ...
        updateInfData(){
            if((this.informationDefine.startAt=='')&&(this.informationDefine.endAt!='')){
                ...
                var arr={
                            ...
                            templetTreeId:this.informationDefine.selectGroup[0].templetTreeId,
                            writeable:this.informationDefine.writeable?1:0,
                            authorityType:this.informationDefine.authorityType,
                            startAt:0,
                            endAt:0,
                ...
                if(this.informationDefine.authorityType==2){
                    ...
                    '/api/api/permission/qzAuthority/update',
                        ...
                        (res)=>{
                            ...
                            quanzongChina=this.getNodeParent(this.informationDefine.selectGroup[0].templetTreeId)
                            for(var i=0;i<this.tableDataInformation.length;i++){
                                console.log('000')
